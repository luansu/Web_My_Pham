<%@ page language="java" contentType="text/html; charset=UTF-8"
	pageEncoding="UTF-8"%>
<!DOCTYPE html>
<html>
<head>
<meta charset="UTF-8">
<title>Insert title here</title>
<link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.2/dist/css/bootstrap.min.css" rel="stylesheet" integrity="sha384-T3c6CoIi6uLrA9TneNEoa7RxnatzjcDSCmG1MXxSR1GAsXEV/Dwwykc2MPK8M2HN" crossorigin="anonymous">
</head>
<body>
	<footer
		class="bg-light-subtle text-center text-lg-start text-dark mt-5 border-top border-success border-opacity-25">
		<!-- Grid container -->
		<div class="container p-4">
			<!--Grid row-->
			<div class="row mt-4">
				<!--Grid column-->
				<div class="col-lg-3 col-md-6 mb-4 mb-md-0">
					<h5 class="text-uppercase">See other books</h5>

					<ul class="list-unstyled mb-0">
						<li><a href="#" class="text-dark text-decoration-none"><i
								class="fas fa-book fa-fw fa-sm me-2"></i>Bestsellers</a></li>
						<li><a href="#" class="text-dark text-decoration-none"><i
								class="fas fa-book fa-fw fa-sm me-2"></i>All books</a></li>
						<li><a href="#" class="text-dark text-decoration-none"><i
								class="fas fa-user-edit fa-fw fa-sm me-2"></i>Our authors</a></li>
					</ul>
				</div>
				<!--Grid column-->

				<!--Grid column-->
				<div class="col-lg-3 col-md-6 mb-4 mb-md-0">
					<h5 class="text-uppercase">Execution of the contract</h5>

					<ul class="list-unstyled mb-0">
						<li><a href="#" class="text-dark text-decoration-none"><i
								class="fas fa-shipping-fast fa-fw fa-sm me-2"></i>Supply</a></li>
						<li><a href="#" class="text-dark text-decoration-none"><i
								class="fas fa-backspace fa-fw fa-sm me-2"></i>Returns</a></li>
						<li><a href="#" class="text-dark text-decoration-none"><i
								class="far fa-file-alt fa-fw fa-sm me-2"></i>Regulations</a></li>
						<li><a href="#" class="text-dark text-decoration-none"><i
								class="far fa-file-alt fa-fw fa-sm me-2"></i>Privacy policy</a></li>
					</ul>
				</div>
				<!--Grid column-->

				<!--Grid column-->
				<div class="col-lg-3 col-md-6 mb-4 mb-md-0">
					<h5 class="text-uppercase">Publishing house</h5>

					<ul class="list-unstyled mb-0">
						<li><a href="#" class="text-dark text-decoration-none">The
								BookStore</a></li>
						<li><a href="#" class="text-dark text-decoration-none">123
								Street</a></li>
						<li><a href="#" class="text-dark text-decoration-none">05765
								NY</a></li>
						<li><a href="#" class="text-dark text-decoration-none"><i
								class="fas fa-briefcase fa-fw fa-sm me-2"></i>Send us a book</a></li>
					</ul>
				</div>
				<!--Grid column-->

				<!--Grid column-->
				<div class="col-lg-3 col-md-6 mb-4 mb-md-0">
					<h5 class="text-uppercase">Write to us</h5>

					<ul class="list-unstyled mb-0">
						<li><a href="#" class="text-dark text-decoration-none"><i
								class="fas fa-at fa-fw fa-sm me-2"></i>Help in purchasing</a></li>
						<li><a href="#" class="text-dark text-decoration-none"><i
								class="fas fa-shipping-fast fa-fw fa-sm me-2"></i>Check the
								order status</a></li>
						<li><a href="#" class="text-dark text-decoration-none"><i
								class="fas fa-envelope fa-fw fa-sm me-2"></i>Join the newsletter</a>
						</li>
					</ul>
				</div>
				<!--Grid column-->
			</div>
			<!--Grid row-->
		</div>
		<!-- Grid container -->

		<!-- Copyright -->
		<div
			class="text-center p-3 bg-body-tertiary border border-success border-opacity-25">
			© 2021 Copyright: <a class="text-info-emphasis" href="#">Kaba.com</a>
		</div>
		<!-- Copyright -->
		<script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.2/dist/js/bootstrap.min.js" integrity="sha384-BBtl+eGJRgqQAUMxJ7pMwbEyER4l1g+O15P+16Ep7Q9Q+zqX6gSbd85u4mG4QzX+" crossorigin="anonymous"></script>
	</footer>
</body>
</html>